  ...
  *)
  ...
  | SOME (Phi, fileref, longids) =>
    let
      ...
      val SOME topbind  =
        SyntaxCheck.find (topbind, (Entity.Fun, funid, Level.topLevel ()))
...
let
  ...
  val (sigma, errors, DE, _) =
    runelab (Entity.Sig, sigid, Level.topLevel ()) (fn () => infSigExp' B sigexp)
in
...
let
  ...
  val ((e,E), errors, DE, psi) =
    runelab (Entity.Str, strid, Level.topLevel ())
    (fn () =>
...
let
  ...
  val (F, errors, DE, psi) =
    runelab (Entity.Fun, funid, Level.topLevel ())
    (fn () =>
         let val _ = TyConstraint.init ()
